...
module.exports = class {
				...
				if (!_.isEmpty(files)) {
					...
					fetchedItems.forEach(file => {
						...
						if (cachedFile) {
							...
							file.files = cachedFile.files ? cachedFile.files : file.files;
	 ...
	 * @private
	 */
	_downloadFile(file, destFolder, opts) {
		...
		return this._getDownloadUrl(cachedFile).then(downloadUrl => {
			...
			let $ = cheerio.load(body);
			let availableFiles = $('#files > ul > li').map(function() {
				let row = $(this);
				return {
					filename: row.find('strong.name').first().html().trim(),
					url: row.find('a.download_button').first().attr('href').trim()
				};
			}).get();
			...
			return Promise.each(filesToDownload, fileToDownload => {
				let opts = {
					url: fileToDownload.url,
					jar: this._ips._cookieJar
				};
				let treatPreperationResult = (readStream, body, filename) => {
					...
					if (waitMsg) {
						...
						logger.info('Waiting %dms...', wait);
						return Promise.delay(wait).then(() => this._prepareDownload(opts).spread(treatPreperationResult))
					}
					// else..
					fs.writeFileSync('download-debug.html', body);
					throw new Error('Unknown error, see download-debug.html.');
				};
				return this._prepareDownload(opts).spread(treatPreperationResult);
			});
		});
	}
	...
	_saveFileCache(categoryId, files) {
		...
		this._fileCache = this._fileCache || { [categoryId]: [] };
		if (categoryId && files) {
			this._fileCache[categoryId] = files;
		}
		this._ips.logger.info('Writing %s file entries for category %s to cache.', this._fileCache[categoryId].length, categoryId);
