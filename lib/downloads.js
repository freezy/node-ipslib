...
Downloads.prototype.findFile = function(query, cat, opts) {
	let regex = getSearchRegex(query);
	return this.getFiles(cat, opts).then(files => _.find(files, f =>
		regex.test(f.title) || regex.test(f.description)
	));
};
...
Downloads.prototype.findFiles = function(query, cat, opts) {
	let regex = getSearchRegex(query);
	return this.getFiles(cat, opts).then(files => _.filter(files, f =>
		regex.test(f.title) || regex.test(f.description)
	));
};
...
Downloads.prototype._downloadFile = function(file, destFolder, opts) {
	...
	return Promise.try(() => {
		...
		return prepareDownload({ url: downloadUrl, jar: this._ips._cookieJar });

	}).spread((readStream, body, filename) => {

		// if have a stream, download file.
		if (readStream) {
			return downloadFile(readStream, filename, destFolder)
		}
		...
		return prepareDownload({ url: binaryUrl, jar: this._ips._cookieJar }).spread((readStream, body, filename) => {

			// we have a stream, so download file.
			if (readStream) {
				return downloadFile(readStream, filename, destFolder)
			}
 ...
 * @returns {Promise.<*[]>}
 */
function prepareDownload(options) {
		...
		req.on('response', response => {
			...
			if (response.statusCode == 200 && response.headers['content-disposition']) {
				...
				response.on('data', function(chunk) {
					...
					resolve([null, buffer.toString(), null]);
 ...
 */
function downloadFile(readStream, filename, destFolder) {
