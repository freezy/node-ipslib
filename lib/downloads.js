...
module.exports = class {
	...
	download(file, destFolder, opts) {
		return Promise.try(() => {
			if (_.isArray(file)) {
				return Promise.each(file, f => this._downloadItem(f, destFolder, opts));
			}
	...
	_downloadItem(file, destFolder, opts) {
		...
		return this._getDownloadUrl(cachedFile).then(downloadUrl => {
			...
			return Promise.each(filesToDownload, fileToDownload => this._downloadFile(fileToDownload));
		});
	}
	...
	_saveFileCache(categoryId, files) {
		...
		this.logger.info('Writing %s file entries for category %s to cache.', this._fileCache[categoryId].length, categoryId);
