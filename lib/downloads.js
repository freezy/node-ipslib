		...
		return Promise.each(filesToDownload, fileToDownload => {
			...
			return Promise.try(() => {
				if (this._opts.waitPeriod) {
					...
					return Promise.delay(this._opts.waitPeriod);
				}

			}).then(() => {
				return this._prepareDownload({ url: fileToDownload.url, jar: this._ips._cookieJar }).spread((readStream, body, filename) => {
