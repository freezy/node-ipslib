...
const Downloads = function Downloads(ipb) {
	this._ibp = ipb;
	this._categoryCache = resolve(ipb._cache, ipb._prefix + '-categories.json');
	this._fileCache = resolve(ipb._cache, ipb._prefix + '-files.json');
};
...
Downloads.prototype.getCategories = function(opts) {
	...
	return Promise.try(() => {
		...
		return this._ibp._get('/index.php?app=downloads').then($ => {
			var categories = $('#idm_categories').find('li > a').filter(function() {
				return /showcat=(\d+)/i.test($(this).attr('href'));
