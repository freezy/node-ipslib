...
Downloads.prototype.getFileDetails = function(file) {
	...
	return this._getDownloadUrl(cachedFile).then(downloadUrl => {
		...
		if (readStream) {
			...
			cachedFile.files = $('#files > ul > li strong.name').html().get();
		}
		this._saveFileCache();
		return cachedFile;
	});
...
Downloads.prototype._getDownloadUrl = function(cachedFile) {
	...
	return Promise.try(() => {
		...
		let info = $('.ipsList_data > li').map(function() {
			let row = $(this);
		...
		cachedFile.description = description ? description.trim() : cachedFile.description;
