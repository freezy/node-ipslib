...
module.exports = class extends Downloads {
		...
		return Promise.try(() => {
			...
			items = items.concat($('.ipsDataList > .ipsDataItem').map((index, el) => {
				...
				let res = {
					...
					id: this._parseIdFromUrl(url),
					title: title,
					description: description ? toMarkdown(description, { gfm: true }).replace(/<[^<]*>/, '').trim() : '',
	...
	_getDownloadUrl(cachedFile) {
		...
		return Promise.try(() => {
			...
			description = toMarkdown($('.ipsPad .ipsType_richText').html(), { gfm: true }).replace(/<[^<]*>/, '').trim();
	...
	_parseFileList($) {
		return $('.ipsDataItem a[data-action="download"]').map((index, el) => {
			...
			return {
				filename: row.find('.ipsDataItem_title').html(),
				url: $(el).attr('href'),
	 ...
	 * @private
	 */
	_downloadFile(fileToDownload) {
		let opts = {
			url: fileToDownload.url,
			jar: this._ips._cookieJar
		};
		return this._prepareDownload(opts).spread(this._treatPreperationResult.bind(this));
	}

	_treatPreperationResult(readStream, body, filename) {
		...
		if (waitMsg) {
			...
			return Promise.delay(wait)
				.then(() => this._prepareDownload(opts)
					.spread(this._treatPreperationResult.bind(this)))
		}
		// else..
